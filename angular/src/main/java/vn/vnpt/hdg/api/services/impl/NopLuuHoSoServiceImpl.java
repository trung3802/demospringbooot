...
@Service
public class NopLuuHoSoServiceImpl implements NopLuuHoSoService {
	...
	@Override
	public AppResponse moHoSoNopLuu(Integer id) {
	    try {        
	...
	@Override
	public AppResponse guiLuuHoSoNopLuu(Integer id) {
	    try {        
	        Optional<TT_Ho_So> existingTT_Ho_SoOptional = rTT_Ho_So.findById(id);
	        if (!existingTT_Ho_SoOptional.isPresent()) {
	            return new AppResponse("Hồ sơ không tồn tại trong hệ thống", false);
	        ...
	        existingTT_Ho_So.setTrangThaiNhan(1);
	        
	        rTT_Ho_So.save(existingTT_Ho_So);
	        
	        return new AppResponse("Gửi hồ sơ thành công", true);
